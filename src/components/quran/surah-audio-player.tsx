...
import { Play, Pause, SkipBack, SkipForward, X, ChevronDown, Mic2 } from "lucide-react";
...
export function SurahAudioPlayer({
  ...
  return (
    <>
      {/* Play Full Surah Button — shown when player is not active */}
      {!isActive && (
        <motion.button
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
          onClick={() => onPlayFullSurah()}
          className="fixed bottom-6 right-6 z-40 flex items-center gap-2 rounded-full bg-gradient-to-r from-emerald-600 to-emerald-700 px-5 py-3 text-white shadow-xl shadow-emerald-900/30 hover:from-emerald-700 hover:to-emerald-800 transition-all active:scale-95"
        >
          <Play className="h-5 w-5 fill-current" />
          <span className="text-sm font-semibold">{t.playFullSurah || "Play Surah"}</span>
        </motion.button>
      )}
              ...
              <div className="mx-auto max-w-2xl px-4 py-3 flex items-center gap-3">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    ...
                    className="flex h-10 w-10 items-center justify-center rounded-full bg-white text-emerald-800 shadow-lg hover:bg-emerald-50 transition-colors active:scale-95"
